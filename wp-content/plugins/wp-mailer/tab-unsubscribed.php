...
class WPM_Table extends WP_List_Table {
    ...
    function column_default( $item, $column_name ) {
        switch( $column_name ) {
            ...
            case 'sub_removed':
                return mysql2date('Y/m/d h:i:s', $item['sub_removed']);
            case 'action':
    ...
    function get_sortable_columns() {
        $sortable_columns = array(
            ...
            'sub_removed'       => array('sub_removed',false)
        );
    ...
    function get_columns(){
        $columns = array(
            ...
            'sub_removed'   => __( 'Unsubscribed', 'wpm_table' ),
    ...
    function column_action($item){
        $actions = array(
            //'edit'          => sprintf('<a href="?page=%s&action=%s&id[]=%s">Edit</a>', WPM_FOLDER.'/tab-unsubscribed.php', 'edit', $item['sub_id']),
            'delete'        => sprintf('<a href="?page=%s&action=%s&id[]=%s">Delete</a>', WPM_FOLDER.'/tab-unsubscribed.php', 'delete', $item['sub_id']),
        );
...
echo '.wp-list-table .column-sub_removed { width: 15%; }';
echo '</style>';

echo '<p>We keep a list of all unsubscribed email addresses, this ensure that if you import a list then these email addresses will not be re-added.</p>';

$wpm_table = new WPM_Table();

$wpm_table->prepare_items();
echo '<form method="post" action="?page='.WPM_FOLDER.'/tab-unsubscribed.php">';
